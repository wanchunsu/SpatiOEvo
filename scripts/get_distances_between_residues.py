...
from pdb_annotations_tools import load_pdb_resolution, download_pdb_file
...
### This script uses data generated from https://github.com/wanchunsu/HV_PPI_interface_evolution (reformatted for use in the analyses presented in this study and stored in the `../data` directory here).
...
def get_distance_between_residues(memo_distances_fi, pdb_resolution_dict, res_type, dnds_info_with_pdb_and_auth_info, pdb_fis_dir, parser):   
    ...
    for targ_prot in dnds_info_with_pdb_and_auth_info:
        ...
        if len(dnds_info_with_pdb_and_auth_info[targ_prot])==1: #only one interfacial residue here, so can't get distance between two residues on this protein
                ...
                for res_pos2 in dnds_info_with_pdb_and_auth_info[targ_prot]:
                    ...
                    if res_pos1 != res_pos2: # we want to look at distance between two DIFF residues (ensure that r1 and r2 aren't the same)
                        ...
                        if len(pdb_chains_to_get_distances_for) > 1: # load structure first b/c will likely need to use more than once
                            download_pdb_file(pdb_with_best_resol.lower(), pdb_fis_dir) # download if doesn't already exist
                            structure = parser.get_structure(pdb_with_best_resol, osp.join(pdb_fis_dir, pdb_with_best_resol.lower() + '.cif'))
                            model = structure[0]
                            model_loaded = True
            ...
            dump_json(memo_distances_dict, memo_distances_fi)
...
def main():
    ...
    dnds_split_by_prots_fi = osp.join(script_dir, '..', 'data', 'dnds', 'targ_prot_res_well_defined_dnds_vals.json')
    ...
    # PDB information
    pdb_resolution_fi = osp.join(script_dir, '..', 'data', 'resolu.idx')

    # PDB files (.cif format)
    pdb_fis_dir = osp.join(script_dir, '..', 'data', 'pdb_files')
    os.makedirs(pdb_fis_dir, exist_ok=True)

    # new dir to output files
    new_dir = osp.join(script_dir, '..', 'data', 'dnds', 'distances_btwn_residues_in_diff_dnds_categories')
