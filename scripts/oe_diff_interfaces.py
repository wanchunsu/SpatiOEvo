...
# Compute O/E contact count ratios (across 10,000 intra-protein node-label randomization trials) for different interfaces and sub-interfaces
...
def shuffle_node_annos(nodes_anno): 
	...
	for tp in nodes_anno:
		...
		if orig_annos_counts!=shuffled_annos_counts: # count for sanity check
			print('Something went wrong... original # of nodes differs from radnomized!')
...
def main():
	...
	dir_of_node_and_edge_annos = osp.join(script_dir, '..', 'data', 'contact_graphs')
	outdir = osp.join(script_dir, '..', 'data', 'contact_graphs', 'interrel')
	
	print('Starting permutation trials to compute O/E . . .')
	
	os.makedirs(outdir, exist_ok = True)
	
	num_trials = 10000 # num permutation trials

	# For each type of interface, create randomized contact graphs (shuffle node labels within proteins, keeping edges intact) num_trial times 
