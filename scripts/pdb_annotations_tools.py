...
def load_pdb_resolution(pdb_resolution_fi):
	...
	''' 
	download_from_url('https://ftp.ebi.ac.uk/pub/databases/pdb/derived_data/index/resolu.idx', pdb_resolution_fi)
	pdb_resolution_dict = {}
...
def download_from_url(url, out_path):
	try:
		if osp.exists(out_path) and osp.getsize(out_path) > 0: #return if exists
			return
		...
		if osp.getsize(out_path) == 0:
			raise RuntimeError("Downloaded file {out_path} is empty!")

		print(f"Downloaded {out_path} :)")

	except Exception as e:
		print(f"Download for {url} failed: {e}!")
...
def download_pdb_files_from_file(list_of_pdb_structs_fi, pdb_dir):
	...
	pdbl = PDBList()
	warnings.filterwarnings("ignore", message="The default download format has changed from PDB to PDBx/mmCif")
...
def download_pdb_file(pdb_id, pdb_dir):
	...
	# Selecting structure from PDB
	pdbl = PDBList()
	warnings.filterwarnings("ignore", message="The default download format has changed from PDB to PDBx/mmCif")
	try:
		if not osp.isfile(osp.join(pdb_dir, pdb_id + '.cif')):
